...
library V4PoolTargetLib {
    ...
    function targetFromTick(V4PoolUtils.PoolSnapshot memory snapshot, int24 targetTick)
        internal
        pure
    ...
    function logTarget(
        ...
        (string memory forward, string memory inverse) =
            V4PoolUtils.tickToPriceStrings(result.targetTick, token0, token1, 6);
        ...
        if (result.zeroForOne) {
            ...
            console2.log(
                string(
                    abi.encodePacked(
                        "Input amount (~)     : ",
                        V4PoolUtils.amountToString(result.inputAmount, token0.decimals, 6),
                        " ",
                        token0.symbol
                    )
                )
            ...
            console2.log(
                string(
                    abi.encodePacked(
                        "Expected output (~)  : ",
                        V4PoolUtils.amountToString(result.outputAmount, token1.decimals, 6),
                        " ",
                        token1.symbol
                    )
                )
            ...
            console2.log(
                string(
                    abi.encodePacked(
                        "Input amount (~)     : ",
                        V4PoolUtils.amountToString(result.inputAmount, token1.decimals, 6),
                        " ",
                        token1.symbol
                    )
                )
            ...
            console2.log(
                string(
                    abi.encodePacked(
                        "Expected output (~)  : ",
                        V4PoolUtils.amountToString(result.outputAmount, token0.decimals, 6),
                        " ",
                        token0.symbol
                    )
                )
    ...
    function _buildResult(V4PoolUtils.PoolSnapshot memory snapshot, int24 targetTick, uint160 targetSqrtPriceX96)
        private
        pure
        returns (TargetResult memory result)
    {
